...
resource "aws_autoscaling_policy" "scale_up" {
  ...
  name                   = "${module.metadata.id}${module.metadata.delimiter}scale${module.metadata.delimiter}up"
...
resource "aws_autoscaling_policy" "scale_down" {
  count                  = local.autoscaling_enabled ? 1 : 0
  name                   = "${module.metadata.id}${module.metadata.delimiter}scale${module.metadata.delimiter}down"
...
locals {
  default_ec2_alarms = {
    cpu_high = {
      alarm_name                = "${module.metadata.id}${module.metadata.delimiter}cpu${module.metadata.delimiter}utilization${module.metadata.delimiter}high"
    ...
    cpu_low = {
      alarm_name                = "${module.metadata.id}${module.metadata.delimiter}cpu${module.metadata.delimiter}utilization${module.metadata.delimiter}low"
...
resource "aws_cloudwatch_metric_alarm" "all_alarms" {
  ...
  alarm_name                = format("%s%s", "${module.metadata.id}${module.metadata.delimiter}", each.value.alarm_name)
